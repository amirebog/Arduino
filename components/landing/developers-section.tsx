...
const codeExamples = [
  {
    label: "Initialize",
    code: `import { Arduino } from '@Arduino/sdk'

const Arduino = new Arduino({
  apiKey: process.env.Arduino_API_KEY
})`,
  },
  {
    label: "Inference",
    code: `const response = await Arduino.inference({
  model: 'Arduino-3-turbo',
  input: userMessage,
  stream: true
})
...
for await (const chunk of response) {
  ...
  {
    label: "Batch",
    code: `const batch = await Arduino.batch.create({
  model: 'Arduino-3',
  inputs: documents,
            ...
            <div className="rounded-xl overflow-hidden bg-card border border-border card-shadow">
              ...
              <div className="border-t border-border p-4 bg-secondary/20">
                <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground mb-2">
                  ...
                  <span>npm install @Arduino/sdk</span>
                </div>
